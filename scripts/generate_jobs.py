...
params = urllib.parse.urlencode({
    'select': 'id,title,company,location,qualification,salary,job_type,description,hr_email,created_at',
    'is_active': 'eq.true',
    'order': 'created_at.desc',
    'limit': '1000',
})
request = urllib.request.Request(
    f'{SUPABASE_URL}/rest/v1/jobs?{params}',
...
def page(job):
    ...
    schema = json.dumps({
        ...
        'jobLocation': {
            ...
            'address': {
                '@type': 'PostalAddress',
                'addressLocality': raw_location,
                'addressCountry': 'IN',
            },
        },
        'employmentType': raw_type.upper().replace(' ', '_'),
        'url': url,
...
document.getElementById("applyButton").addEventListener("click", async function () {
  const r = await sb.from("jobs").select("id,title,company,location,qualification,salary,job_type,description,hr_email,created_at").eq("id", __JOB_ID__).eq("is_active", true).single();
  if (r.error || !r.data) {
...
'''
    return (html
        ...
        .replace('__QUALIFICATION__', qualification)
