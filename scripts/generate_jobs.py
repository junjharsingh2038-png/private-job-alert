...
for job in jobs:
    name = job_slug(job); active.add(name); url = f'{BASE}/jobs/{name}/'; folder = JOBS_DIR / name; folder.mkdir(parents=True, exist_ok=True); (folder / 'index.html').write_text(page(job), encoding='utf-8'); urls.append(url); job_links.append((url, clean_text(job.get('title'), 'Private Job'), clean_text(job.get('company'), 'Employer'), clean_text(job.get('location'), 'India'), clean_text(job.get('qualification'), 'Any qualification')))
