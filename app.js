...
async function loadJobs(){
  ...
  $("jobsList").innerHTML = data.length ? data.map(j=>`
    <article class="job">
      ...
      <h3>${esc(j.title)}</h3><b>${esc(j.company||j.company_name||"")}</b>
...
window.openApply = function(job){
  currentJob=job;
  openModal(`<h2>Apply for ${esc(job.title)}</h2><p><b>${esc(job.company||job.company_name||"")}</b> · ${esc(job.location||"Rajasthan")}</p>
  ...
  <button type="submit">Submit Application</button><p id="applyMsg"></p></form>`);
...
async function submitApplication(e){
  ...
  try {
    ...
    const company=currentJob.company||currentJob.company_name||"";
    const application={job_id:currentJob.id,hr_email:currentJob.hr_email,candidate_name:fd.get("name"),candidate_mobile:fd.get("mobile"),candidate_email:fd.get("email"),candidate_location:fd.get("location"),qualification:fd.get("qualification"),experience:fd.get("experience")||""};
    const saved=await sb.from("applications").insert(application);
    if(saved.error) throw new Error("Application could not be saved. Please try again.");
    ...
    msg.className="success";msg.textContent="Application submitted successfully. HR has been notified by email.";form.reset();
...
window.authForm = function(mode){
  ...
  <form id="authForm" class="form"><label>Email<input type="email" name="email" required></label><label>Password<input type="password" name="password" minlength="6" required></label>
  ${mode==="signup"?'<label>Company Name<input name="company" required></label>':''}<button>${mode==="login"?"Login":"Create Account"}</button><p id="authMsg"></p></form>`);
...
window.showHrPanel = async function(){
  ...
  if(jobsError){openModal(`<h2>HR Dashboard</h2><p class="danger">Your HR account is logged in, but jobs could not be loaded.</p><button onclick="sb.auth.signOut().then(closeModal)">Logout</button>`);return;}
  ...
  <h3>Your Jobs</h3>${(jobs||[]).map(j=>`<div class="job"><b>${esc(j.title)}</b><br>${esc(j.company||j.company_name||"")} · ${esc(j.location||"")}<br><small>${j.is_active!==false?"Active":"Inactive"}</small> <button onclick="deleteJob('${j.id}')">Delete</button></div>`).join("")||"<p>No jobs yet.</p>"}
  <h3>Applications Received</h3>${(apps||[]).map(a=>`<div class="job"><b>${esc(a.candidate_name)}</b><br>📱 ${esc(a.candidate_mobile)} · ✉️ ${esc(a.candidate_email)}<br>🎓 ${esc(a.qualification||"")} · ${esc(a.experience||"Fresher")}<br><small>${new Date(a.created_at).toLocaleString()}</small></div>`).join("")||"<p>No applications yet.</p>"}`);
}

window.showPostJob = function(){
  openModal(`<h2>Post a Job</h2><form id="jobForm" class="form">
  <label>Job Title<input name="title" required></label><label>Company<input name="company" required></label><label>Location<input name="location" required></label>
  <label>Qualification<input name="qualification"></label><label>Salary<input name="salary"></label><label>Job Type<input name="job_type" value="Full Time"></label>
  <label>Last Date<input name="lastdate" placeholder="DD-MM-YYYY"></label><label>Job Description<textarea name="description" rows="5"></textarea></label>
  ...
  $("jobForm").addEventListener("submit",async e=>{
    ...
    if(!user){msg.className="danger";msg.textContent="Please login again.";return;}
    const company=f.get("company");
    const row={hr_user_id:user.id,user_id:user.id,owner_id:user.id,hr_email:user.email,title:f.get("title"),company,company_name:company,location:f.get("location"),qualification:f.get("qualification"),salary:f.get("salary"),job_type:f.get("job_type"),type:f.get("job_type"),lastdate:f.get("lastdate")||null,description:f.get("description"),is_active:true};
    const r=await sb.from("jobs").insert(row);
    if(r.error){msg.className="danger";msg.textContent=r.error.message;return;}
...
function bindButtons(){
  const login=$("hrLoginBtn"),create=$("createHrBtn");
  if(login) login.onclick=()=>window.authForm("login");
  if(create) create.onclick=()=>window.authForm("signup");
}
